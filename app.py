...
s3 = boto3.client(
    's3',
    aws_access_key_id=os.get("AWS_ACCESS_KEY"),
    aws_secret_access_key=os.get("AWS_SECRET_KEY"),
...
@app.route('/')
def index():
    primary_files   = list_files(os.get("PRIMARY_BUCKET"))   or []
    secondary_files = list_files(os.get("SECONDARY_BUCKET")) or []
    ...
    return render_template_string(HTML,
                                  primary_bucket=os.get("PRIMARY_BUCKET"),
                                  secondary_bucket=os.get("SECONDARY_BUCKET"),
...
def upload():
    ...
    try:
        ...
        s3.put_object(Bucket=os.get("PRIMARY_BUCKET"),   Key=filename, Body=file_content)
        # Replicate to secondary bucket (Brick 2)
        s3.put_object(Bucket=os.get("SECONDARY_BUCKET"), Key=filename, Body=file_content)
...
def download(filename):
    ...
    try:
        file_obj = io.BytesIO()
        s3.download_fileobj(os.get("PRIMARY_BUCKET"), filename, file_obj)
        file_obj.seek(0)
    ...
    try:
        file_obj = io.BytesIO()
        s3.download_fileobj(os.get("SECONDARY_BUCKET"), filename, file_obj)
        file_obj.seek(0)
...
def delete(filename):
    try:
        s3.delete_object(Bucket=os.get("PRIMARY_BUCKET"),   Key=filename)
        s3.delete_object(Bucket=os.get("SECONDARY_BUCKET"), Key=filename)
